...
核心目标： 设计一个中继服务（透明转发器），接收业务系统的通知请求，尽可能可靠地投递到目标地址（at-least-once）。

服务定位：透明转发器

中继服务不理解也不转换请求内容，只负责"可靠送达"：

业务系统（组装完整请求）──▶ 中继服务（透明转发 + 重试保障）──▶ 外部供应商 API

- 业务系统的责任：按各供应商的 API 规范，组装好完整的 URL、Headers、Body 后提交给中继服务
- 中继服务的责任：将收到的请求原样 POST 到指定 URL，确保投递成功（重试直到 200 或耗尽次数）
- 中继服务不做：不解析 body 内容、不做格式转换、不维护供应商配置模板

这样中继服务保持极简通用，新增供应商时只需业务系统侧适配，中继服务无需任何改动。

需求边界

- 上游请求协议统一，不需要适配不同结构
- 下游格式适配由业务系统完成：业务系统提交时已组装好完整的 targetUrl/headers/body，中继服务只做透明转发
- 链路中必须携带唯一幂等 ID
